...
- **M3** 书籍层。前半段是 2026-08-04 用户报的进度 bug + 新需求(见「进度模型 v2」),后半段是原定内容:
  - **a 进度模型 v2 数据层**:`cursor` / `done[]` / `skipped[]` 拆分 + 旧书迁移 + 箭头只动光标(**修掉"往回翻进度清零"**)+ 结算写 `done`
  - **b 书卡**:百分比改成白字占比、进度条标出缺口、缺口胶囊(≤6)+ ✗ + 点胶囊跳过去打
  - **c 打字页**:已结算的词显示白色、「回到我的进度」控件、缺口 >6 时 refresh 先弹胶囊窗
