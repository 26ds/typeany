...
- **白 / 灰**:结算过 = 白;没打的、跳过的 = 灰(v2 时期还有「refresh 掉没结算完的」一类,v3 起重打不再改白灰)。这套配色在书卡进度条和打字页正文里是同一套语言
...
- 光标所在窗口有重叠的重打记录时,右上角一个数字一条;点数字查看那一次:
  - **没打完的** → 正文标出上次打到哪 + 「接着继续」;继续时用时累计,time 模式按**剩余时间**接着倒数
  - **打完的** → 整段灰色只读(这是记录,不能再打),不用点就直接显示「开始时刻(年月日时分秒)→ 结束时刻 · 用时」,右下角「战绩」→ 那一次的结算页

**回看模式(打字页):**

- 光标落在全白一段且没开打 = 回看:不计时、**整段完整显示、可上下滚动**(选 100/200 词一轮时必须;正在打字仍只显示三行保持专注)
- 回看中敲字母键即切入重打;重打进行中正文按正常打字配色走(灰→打对变白),全白只在回看态,否则看不出打到哪
