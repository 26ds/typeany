...
| `done[]` 成绩 | **已结算**的词区间集合(有序、相邻自动合并) | 只有「结算」会往里加;`reset progress` 会清空 |
| `skipped[]` 忽略 | 用户 ✗ 掉的缺口 | ✗ 加,重新打完则移除 |

- **左右箭头只动 `cursor`,永远不碰 `done`。** 任何操作都不得让百分比下降 —— 除了用户自己按 `reset progress`
- **「结算」的定义** = 出了结算页,包含两种:自然打完一轮、shift+enter 提前结束。**refresh(重开)不算**,这一轮打过的字一个都不记
- **白 / 灰**:结算过 = 白;没打的、跳过的、refresh 掉没结算完的 = 灰。这套配色在书卡进度条和打字页正文里是同一套语言
- **百分比 = 白字 ÷ 全书词数**(不再是"光标位置 ÷ 词数")
- **进度线 frontier** = `done` 里最靠后的结尾。它只增不减
...
- 光标 ≠ 进度线时,打字页出现一个「回到我的进度」控件 —— 不管用户是点胶囊回来重打的,还是自己按箭头翻回来的,都是这一个控件
