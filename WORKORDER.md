...
- **回合设置 = 顶部胶囊条,和 Random Mode 同一套控件**(用户定 2026-07-31):跟 random 页那排 `time / words + 档位` 长得一样、点法一样,不要藏进设置里。默认按**字数**、给一个默认档;用户可当场切成时间或换字数档。变更在"下一轮"生效(结算后、开打前调整 → 之后每轮沿用新值)。默认档位数值见「待确认」#8
...
- **M2 进行中** custom 弹窗裁剪 + saved texts 升级书架 → 详见 `docs/plans/M2.md`。**a 弹窗裁剪 ✅(2026-07-31)/ b 本地书籍数据层 + 书架页 ✅(2026-07-31)/ c 书架操作闭环 + 虚线上传卡 + 本地多格式文本提取 ⬜**(2026-07-31 扩围:原定只做 .txt/粘贴,用户要求上传卡直接吃 PDF/DOCX/MD)
- **M3** 书籍层:**顶部回合胶囊条(同 random 样式)**、指针连续性引擎、双进度条、**左右箭头切上/下一段**、双结算、顺序 + 章节/页随机、符号跳过;热力图本地记数开始
...
8. **书籍打字页回合胶囊的默认档位**(2026-07-31 提出):默认按字数,但"默认多少字"和档位表未定。候选:`25 / 50 / 100 / 200` 词,默认 50;时间档沿用 random 的 `15 / 30 / 60 / 120` 秒 → **待用户拍板**
